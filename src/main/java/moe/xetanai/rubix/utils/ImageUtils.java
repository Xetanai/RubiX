...
public class ImageUtils {
    ...
    public ImageUtils drawShapeMaskedImage (@Nonnull Rectangle2D bounds, @Nonnull BufferedImage cropImage, @Nonnull Shape cropShape) {
	    ...
	    this.g.drawImage(img, (int) bounds.getX(), (int) bounds.getY(), (int) bounds.getWidth(), (int) bounds.getHeight(), null);
    ...
    public ImageUtils drawText (@Nonnull Rectangle2D bounds, @Nonnull String text, @Nullable Color color) {
        ...
        this.g.setColor(color);
        this.g.drawString(text, (int) bounds.getX(), (int) (bounds.getY() + bounds.getHeight()) - offset);
        return debugDrawBounds(bounds);
    }
    ...
    public ImageUtils drawCenteredText (@Nonnull Rectangle2D bounds, @Nonnull String text, @Nullable Color color) {
        ...
        this.g.setColor(color);
        this.g.drawString(text, (int) bounds.getX() + xoffset, (int) (bounds.getY() + bounds.getHeight()) + yoffset);
        return debugDrawBounds(bounds);
    }
    ...
    private ImageUtils scaleFontToBounds (@Nonnull String text, @Nonnull Rectangle2D bounds) {
	    ...
	    this.g.setRenderingHint(KEY_FRACTIONALMETRICS, VALUE_FRACTIONALMETRICS_ON);

        // Get the current font bounds
        Font cf = this.g.getFont();
        Rectangle2D cbounds = this.g.getFontMetrics(cf).getStringBounds(text, this.g);

        // Scale the font to fit horizontally
        Font nf = cf.deriveFont((float) (cf.getSize2D() * bounds.getWidth() / cbounds.getWidth()));

        // Scale the font to fit vertically, if necessary
        cbounds = this.g.getFontMetrics(nf).getStringBounds(text, this.g);
        if (cbounds.getHeight() > bounds.getHeight()) {
            nf = nf.deriveFont((float) (nf.getSize2D() * bounds.getHeight() / cbounds.getHeight()));
        }
    ...
    private ImageUtils debugDrawBounds (@Nonnull Rectangle2D bounds) {
        ...
        logger.trace("{} | Bounds: X={}, Y={}, W={}, H={}", this.name, bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight());
        return this;
    }
    ...
    private static Font initFont () {
        try {
            InputStream is = ImageUtils.class.getClassLoader().getResourceAsStream(DEFAULT_FONT_NAME);
            if (is == null)
                throw new IOException("Font was null.");
